...
begin
    return (
        with discussion_ as (
            select collectivite_id, action_id
...
begin
    return (
        with audit_ as (
            select collectivite_id, referentiel
...
-- Table dcp
drop policy own_dcp_only on dcp;
...
alter policy allow_insert on indicateur_commentaire with check (
        have_edition_acces(collectivite_id) or private.est_auditeur(collectivite_id));
alter policy allow_update on indicateur_commentaire using (
        have_edition_acces(collectivite_id) or private.est_auditeur(collectivite_id));
...
alter table indicateur_personnalise_definition alter column collectivite_id set not null;
...
select
    case when can_read_acces_restreint(plan_action_tableau_de_bord.collectivite_id) then
...
WHERE collectivite_niveau_acces.niveau_acces IS NOT NULL or collectivite_niveau_acces.est_auditeur
